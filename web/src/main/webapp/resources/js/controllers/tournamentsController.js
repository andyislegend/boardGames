angular.module('homeApp').controller("showAllTournamentsCtrl", function ($scope, $rootScope, $http, $routeParams) {
    ...
    $scope.isContains = function(username){
    	for(var tournament in $scope.tournamentParticipants){
    		if(tournament.username === username) {
    			return true;
    		}else{
    			return false;
    		} 		
    ...
    $scope.changeStatusToRate = function() {
    	$http.put("/setOpportunityForRate/"+$scope.tournament.id).success(function(result){     	
        });
    }
});